...
void tSUBP::parseParameters(){
	...
	if (globalCurrentLine[currentCharIterator] != '('){
		cout << "\t\t\t[SUBP]: Invalid Syntax: Expecting ( Operation: " << globalCurrentLine << endl;
		globalNumErrors++;
	...
	while (continueParsingVariable){
		...
		if (currentChar == ')'){
			...
			if (parameterNumber == VARIABLE_ID_CODE){
				cout << "\t\t\t[SUBP]: Invalid Syntax: Expecting ID before ) : " << globalCurrentLine << endl;
				globalNumErrors++;
			}
		...
		if (!caseFound){
			cout << "\t\t\t[SUBP]: Invalid Syntax: Unknown Character in line: " << globalCurrentLine << endl;
			(*currentCharIterator)++;
			globalNumErrors++;
		}
	}
...
bool tSUBP::parseConstant(int *currentCharIterator, int parameterNumber){
	...
	while (continueParsingVariable){
		...
		if (currentChar == '\0'){
			if (numCharactersInVarName == 0){
				cout << "\t\t\t[SUBP]: Invalid Syntax: Expecting (<ID> <globalOperation> <ID>) then <lineLabel>: " << globalCurrentLine << endl;
				isValidVariableName = false;
				globalNumErrors++;
			}
			continueParsingVariable = false;
...
void tSUBP::parseOperation(int *currentCharIterator){
	...
	operationString = globalCurrentLine.substr(4, 7);
	caseFound = false;
	// cout << "\t\t\t[SUBP]: Current Character: " << currentChar << endl;

	if (operationString == "SIN"){
		// cout << "\t[Compiler]: Found SIN Command\n";
	...
	if (operationString == "COS"){
		// cout << "\t[Compiler]: Found COS Command\n";
		(*currentCharIterator) += 3;
		caseFound = true;
	...
	if (operationString == "EXP"){
		// cout << "\t[Compiler]: Found EXP Command\n";
		(*currentCharIterator) += 3;
		caseFound = true;
	...
	if (operationString == "ABS"){
		// cout << "\t[Compiler]: Found ABS Command\n";
		(*currentCharIterator) += 3;
		caseFound = true;
	...
	if (operationString == "ALG"){
		// cout << "\t[Compiler]: Found ALG Command\n";
		(*currentCharIterator) += 3;
		caseFound = true;
	...
	if (operationString == "ALN"){
		// cout << "\t[Compiler]: Found ALN Command\n";
		(*currentCharIterator) += 3;
		caseFound = true;
	...
	if (operationString == "LOG"){
		// cout << "\t[Compiler]: Found LOG Command\n";
		(*currentCharIterator) += 3;
		caseFound = true;
	...
	if (operationString == "SQR"){
		// cout << "\t[Compiler]: Found SQR Command\n";
		(*currentCharIterator) += 3;
		caseFound = true;
	...
	if (!caseFound){
		cout << "\t\t\t[SUBP]: Invalid Syntax: Unable to Parse Operation: " << globalCurrentLine << endl;
		globalNumErrors++;
	}
