...
void GOTO::prepareGOTO(FileManager *parentFileManager, LineLabelTable *parentLineManager){
...
int GOTO::handleGOTO(string currentLine, int correspondingLineNumber){
...
bool GOTO::parseLineLabelName(int *currentCharIterator){
	char currentChar;
...
void GOTO::outputGOTOCommand(){
	unsigned int currentMemoryLocation;
	// cout << "\t\t[GOTO]: Attempting to Print Object code to .obj...\n";

	(*currentFileManager).writeCharToObj(GOTO_OP_CODE);
	(*currentFileManager).writeCharToObj(' ');
	(*currentFileManager).writeNumToObj(globalLineLabelLineNumber);
