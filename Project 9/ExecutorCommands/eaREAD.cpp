...
// calls the functions necessary to execute the aREAD command
void eaREAD::handleAREAD(ProgramLineObject *currentLine){
	...
	// cout << "\t\t[aREAD]: Executing aREAD command for array at " << arrayMemoryLocation_int << " between indexes from mem locations " << startIndexMemLocation_int << " to " << endIndexMemLocation_int << endl;
	...
	// cout << "\t\t[aREAD]: Executing aREAD command for array at " << arrayMemoryLocation_int << " for indexes " << startIndexValue_string << " to " << endIndexValue_string << endl;
	...
	if ((*ParentMemoryManager).rangeChecksOut(arrayMemoryLocation_int, startIndexValue_int, endIndexValue_int)){
		// cout << "\t\t[aREAD]: Executing aREAD command for array at " << arrayMemoryLocation_int << " for indexes " << startIndexValue_int << " to " << endIndexValue_int << endl;
		executeAREADCommand(arrayMemoryLocation_int, startIndexValue_int, endIndexValue_int);
	}
	else {
		cout << "\t\t[aREAD]: Boundary Error for array at " << arrayMemoryLocation_int << " for indexes " << startIndexValue_int << " to " << endIndexValue_int << endl;
	}
...
void eaREAD::executeAREADCommand(int arrayMemoryLocation, int startIndex, int endIndex){
	// cout << "\t\t[eaREAD]: Attempting to Execute Object Code...\n";
	string tempValue;
	int currentIndexIntoCoreMemory = arrayMemoryLocation + startIndex;

	for (int i = startIndex; i <= endIndex; i++){
		cin >> tempValue;
		(*ParentMemoryManager).setValue(currentIndexIntoCoreMemory, tempValue);
		// cout << "\t\t[eaREAD]: Saved \"" << tempValue << "\" to location " << currentIndexIntoCoreMemory << endl;
