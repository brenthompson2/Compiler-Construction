...
void LOOP::prepareLOOP(FileManager *parentFileManager, SymbolTable *parentMemoryManager){
...
// returns num errors
int LOOP::handleLOOP(string currentLine, int correspondingLineNumber){
...
bool LOOP::parseVariable(int *currentCharIterator, int parameterNumber){
	char currentChar;
...
// parses through a line one character at a time, manages the global member variable associated with the parameterNumber, and returns whether or not there are any more parameters to parse
bool LOOP::parseConstant(int *currentCharIterator, int parameterNumber){
	char currentChar;
	string currentVariableName = "";
	int numCharactersInVarName = 0;
...
void LOOP::syncVariablesToSymbolTable(){
